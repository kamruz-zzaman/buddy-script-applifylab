...
export default function FeedClient({ children }) {
  ...
  useEffect(() => {
    ...
    const timelineDropShowBtn = document.querySelector(
      "#_timeline_show_drop_btn",
    );
    ...
    const notifSettingsBtn = document.querySelector(
      "._notification_box_right_link",
    );
    const notifSettingsDrop = document.querySelector(
    ...
    document.addEventListener("click", function (e) {
      if (
        profileDropdown &&
        !e.target.closest("#_prfoile_drop") &&
        !e.target.closest("#_profile_drop_show_btn")
      ) {
        profileDropdown.classList.remove("show");
        isDropShow = false;
      }
      if (
        timelineDropdown &&
        !e.target.closest("#_timeline_drop") &&
        !e.target.closest("#_timeline_show_drop_btn")
      ) {
        timelineDropdown.classList.remove("show");
      ...
      if (
        notifyDropdown &&
        !e.target.closest("#_notify_drop") &&
        !e.target.closest("#_notify_btn")
      ) {
        notifyDropdown.classList.remove("show");
        isDropShow1 = false;
      }
